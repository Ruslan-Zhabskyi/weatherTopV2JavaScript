...
export const stationController = {
  async index(request, response) {
    ...
    if (latestReading != null) {
      latestReading.temperatureInF = await conversions.convertToFahrenheit(
        latestReading.temperature
      );
      latestReading.weatherCodeDecrypted = await conversions.weatherCodes(
        latestReading.code
      );
      latestReading.windBft = await conversions.windSpeedBeaufortConversion(
        latestReading.windSpeed
      );
      latestReading.windBftLabel =
        await conversions.windSpeedBeaufortConversionLabel(
          latestReading.windSpeed
        );
      latestReading.windChill = await conversions.windChillCalculator(
        latestReading.temperature,
        latestReading.windSpeed
      );
