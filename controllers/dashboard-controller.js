...
export const dashboardController = {
  async index(request, response) {
    ...
    for (const station of stations) {
      const latestReading = await stationAnalytics.getLatestReading(
        station._id
      );
      ...
      if (latestReading != null) {
        ...
        station.temperatureInF = await conversions.convertToFahrenheit(
          latestReading.temperature
        );
        station.weatherCodeDecrypted = await conversions.weatherCodes(
          latestReading.code
        );
        station.windBft = await conversions.windSpeedBeaufortConversion(
          latestReading.windSpeed
        );
        station.windBftLabel =
          await conversions.windSpeedBeaufortConversionLabel(
            latestReading.windSpeed
          );
        station.windChill = await conversions.windChillCalculator(
          latestReading.temperature,
          latestReading.windSpeed
        );
        station.windDirection =
          await conversions.windDirectionCompassConversion(
